...
ruido completa, via pl4) com uma ou mais malhas de features pre-extraidas
(v10: alta impedancia em distancia x angulo x rfault, remote fixo em 200km;
v11: mesma ideia mas variando tambem o remote). Os pl4 dessas malhas foram
apagados apos a extracao para economizar espaco, entao essas amostras
entram so na condicao 'ideal', sem as 6 variantes de ruido."""
...
def main() -> int:
    ...
    parser.add_argument("--dense-grid-dir", type=Path, action="append", required=True,
                         help="Pode ser passado varias vezes para combinar multiplas malhas.")
    ...
    print(f"\nTotal treino: {len(y_train)} amostras ({len(y_train) - dense_grid_count} com augmentacao "
          f"de ruido do manifesto + {dense_grid_count} das malhas densas, condicao ideal)")
