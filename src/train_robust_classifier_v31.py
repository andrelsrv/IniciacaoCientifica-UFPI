...
def main() -> int:
    ...
    dense_x = [x_manifest]
    dense_y = [y_manifest]
    # rfault de cada amostra, para filtrar os especialistas pela faixa
    # dificil (ver secao de especialistas abaixo). O manifesto e' sempre
    # baixa-Z para faltas com terra (~75% abaixo de 150ohm, confirmado),
    # entao 0.0 e' um placeholder seguro -- nunca entra na faixa "dificil"
    # (>=300ohm) usada pelos especialistas AB/ABG e BC/BCG.
    dense_rf = [np.zeros(len(y_manifest))]
    dense_grid_count = 0
    ...
    # AB/ABG e BC/BCG receberam peso extra pro lado franca: o teste barato
    # do v27 mostrou que mesmo com class_weight="balanced_subsample" (que
    # so equaliza contagem de amostras) o especialista ainda favorecia a
    # versao aterrada nessas duas duplas, piorando AB (56%->51%) e BC
    # (67%->61%) mesmo melhorando CA/CAG. Isso indica vies pela SOBREPOSICAO
    # de features (nao contagem), entao usamos peso extra explicito alem do
    # balanceamento padrao. CA/CAG nao recebe boost -- ja funcionava bem.
    # CAP no lado aterrado (max 5x o tamanho do lado franca): a malha v30
    # trouxe 10.800 casos novos de CAG contra so 75 de CA (bug na geracao
    # de reforco fase-fase), inflando a proporcao CAG:CA de ~1:1 para
    # ~9:1. Mesmo com class_weight="balanced_subsample", esse desequilibrio
    # extremo diluiu a representacao de CA nos bootstraps de cada arvore e
    # piorou o especialista (CA 94.7%->87.5% no teste de 12k). Subamostrar
    # o lado aterrado mantem a informacao nova (mais variedade de Rfault)
    # sem deixar CA praticamente invisivel no treino.
    # NOVO nesta rodada: AB/ABG e BC/BCG treinam so com o lado aterrado na
    # faixa DIFICIL (Rfault>=300ohm), nao com o espectro completo de ABG/BCG.
    # Motivacao: um classificador binario pequeno treinado so nos casos
    # dificeis (AB franco vs ABG em 900-3000ohm) chegou a 79.5% de
    # validacao cruzada -- muito acima do especialista anterior (treinado
    # com TODO o ABG, a maioria facil/baixa-Z), que so conseguiu 51% (pior
    # que nao usar especialista nenhum, 56%). Hipotese: quando a maioria
    # dos exemplos de ABG no treino e' facil (baixa-Z, bem diferente de
    # AB), a arvore aprende o padrao tipico de ABG em vez da fronteira
    # sutil especifica com AB franco -- que e' um caso raro dentro do
    # proprio ABG. Restringir ao subconjunto dificil forca o especialista
    # a focar exatamente nessa fronteira. CA/CAG mantem o espectro
    # completo -- ja funcionava bem (94-97%) sem esse filtro.
    MAX_GROUNDED_RATIO = 5
    HARD_ZONE_MIN_RFAULT = {"AB": 300.0, "BC": 300.0, "CA": 0.0}
    specialist_pairs = (("AB", "ABG", 2.0), ("BC", "BCG", 2.0), ("CA", "CAG", 1.0))
    specialists = {}
    for cls_a, cls_b, boost in specialist_pairs:
        rng_spec = np.random.RandomState(20260815)
        idx_a = np.flatnonzero(y_combined == cls_a)
        min_rf = HARD_ZONE_MIN_RFAULT[cls_a]
        idx_b = np.flatnonzero((y_combined == cls_b) & (rfault_combined >= min_rf))
        if min_rf > 0:
            print(f"  {cls_a}/{cls_b}: usando so {cls_b} com Rfault>={min_rf}ohm "
                  f"({len(idx_b)} de {int((y_combined == cls_b).sum())} disponiveis)")
        max_b = len(idx_a) * MAX_GROUNDED_RATIO
        if len(idx_b) > max_b:
            idx_b = rng_spec.choice(idx_b, size=max_b, replace=False)
            print(f"  {cls_a}/{cls_b}: subamostrando {cls_b} de volta para {max_b} "
                  f"(era {len(np.flatnonzero(y_combined == cls_b))}, max {MAX_GROUNDED_RATIO}x {cls_a})")
        idx_pair = np.concatenate([idx_a, idx_b])
        x_pair = x_combined[idx_pair]
        y_pair = y_combined[idx_pair]
        sample_weight = np.where(y_pair == cls_a, boost, 1.0)
        ...
        spec.fit(x_pair, y_pair, sample_weight=sample_weight)
        specialists[f"{cls_a}_{cls_b}"] = spec
        print(f"  especialista {cls_a}/{cls_b}: {len(idx_pair)} amostras de treino, "
              f"peso {boost}x para {cls_a}")
