"""Validacao final independente do classificador v5 (1146 casos de treino,
reforco de curta distancia nas classes sem terra). Lote totalmente novo."""
...
OUTPUT_ROOT = Path(r"C:\RESULTPESQUISA\quick_check_v5_final")
CLASSIFIER_V5 = Path(r"C:\RESULTPESQUISA\campaign_v6_ab_focus\classifier_robust_v5\robust_classifier.joblib")
...
def main() -> None:
    ...
    n = len(results)
    accuracy = sum(r["hit"] for r in results) / n
    g = [r for r in results if r["true_class"] in GROUNDED]
    u = [r for r in results if r["true_class"] not in GROUNDED]
    print(f"\nAcuracia geral (v5, {n} casos novos): {accuracy:.1%}")
    print(f"Com terra: {sum(r['hit'] for r in g)}/{len(g)} = {sum(r['hit'] for r in g)/len(g):.1%}")
    print(f"Sem terra: {sum(r['hit'] for r in u)}/{len(u)} = {sum(r['hit'] for r in u)/len(u):.1%}")

    report_path = OUTPUT_ROOT / "quick_check_v5_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        json.dumps({"accuracy": accuracy, "results": results}, indent=2, ensure_ascii=False), encoding="utf-8"
    )
